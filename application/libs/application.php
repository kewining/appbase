...
class Application
{
    ...
    private function splitUrl()
    {
        if (isset($_GET['url'])) {
            ...
            // split URL
            $url = rtrim($_GET['url'], '/');//Remove the blanks
            $url = filter_var($url, FILTER_SANITIZE_URL);//filter with a variable which prompted
            $url = explode('/', $url);//Split a string into multiple strings '/'

            $this->url_controller = (isset($url[0]) ? $url[0] : null);//controller
			/* ope ternario
			 * if(isset($url[0]))
			 * { $url[0] }
			 * else { null }
			 */
            $this->url_action = (isset($url[1]) ? $url[1] : null);//methods
            $this->url_parameter_1 = (isset($url[2]) ? $url[2] : null);//parameter
